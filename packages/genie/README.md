...
Wire-format types (`ToolProgressEvent`, `ToolProgressPhase`) live in [`@dbx-tools/appkit-genie-shared`](../genie-shared) and are re-exported from this package, so a single import gets you both the runtime and the contracts:
