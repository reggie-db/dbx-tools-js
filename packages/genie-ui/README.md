...
React UI for [`@dbx-tools/appkit-genie`](../genie). Drops in an `<AgentChat>` component that renders an agent's streaming response plus live tool-progress updates, styled to match [`@databricks/appkit-ui`](https://www.npmjs.com/package/@databricks/appkit-ui)'s `<GenieChat>` so the two can sit side by side.

This package has **no runtime dependency on the server plugin**. Shared wire-format types (`ToolProgressEvent`, `ToolProgressPhase`) come from [`@dbx-tools/appkit-genie-shared`](../genie-shared), so the UI can be bundled into a browser without dragging in Node-only AppKit code.
